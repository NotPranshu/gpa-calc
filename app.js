...
function confirmScannerImport() {
  ...
  if (!rows.length || result.semesterGpa === null) {
    setScannerStatus("Unable to detect grades. Please upload a clearer image.", "warning");
    return;
  }

  const semesterLabel = normalizeSemesterLabel(scannerSemesterLabel?.value || "", (academicProfile?.semesters?.length || 0) + 1);
  ...
  academicProfile.semesters = [...(academicProfile?.semesters || []), semesterRecord];
  semesterHistory = academicProfile.semesters.map((semester, index) => ({
    ...
    createdAt: semester.createdAt || Date.now() + index,
  }));
...
async function handleScannerFile(file) {
  ...
  scannerExtractedConfidence = null;
  renderScannerReview([]);
  if (scannerScanBtn) scannerScanBtn.disabled = false;
  setScannerStatus(`Selected ${file.name}. Click Scan Results to extract courses.`, "neutral");
}

function toggleScannerEditMode() {
  if (!scannerExtractedRows.length) return;
  scannerEditMode = !scannerEditMode;
  applyScannerEditMode();
}
